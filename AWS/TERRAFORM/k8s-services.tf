...
# Kubernetes Service for MKPublisher
resource "kubernetes_service_v1" "mkpublisher" {
  metadata {
    name      = "mkpublisher-service"
    namespace = "default"
...
output "mkpublisher_loadbalancer_hostname" {
  description = "Hostname of the MKPublisher Load Balancer"
  value       = try(kubernetes_service_v1.mkpublisher.status[0].load_balancer[0].ingress[0].hostname, null)
}
